...
def consonance():
    ...
    wavfile.write(
        "data/major_thirds.wav", rate=44100, data=((C4 + E4) / 2).astype(np.int16)
    )
    ...
    wavfile.write(
        "data/minor_seconds.wav", rate=44100, data=((C4 + c4) / 2).astype(np.int16)
    )
