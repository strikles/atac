...
emergency_email_file = (
    "atac/tests/contacts/emergency_used_only_whilst_under_torture.csv"
)
...
def test_send_email():
    ...
    message_content = katie.get_file_content(
        os.path.abspath(
            os.path.join(os.getcwd(), "data/messages/email/neurorights.md")
        ),
        "message",
    )
...
def test_send_emails_with_no_paraphrasing_and_no_translation():
    ...
    message_file_path = os.path.join(
        os.getcwd(), "data/messages/email/neurorights_en.md"
    )
